...
To get the number of bits set in {\texttt{a}} better known as ``popcount'' (population count):
\index{mp\_popcount}
\begin{alltt}
int mp_popcount(const mp_int *a)
\end{alltt}

To get the quantitative difference between two {\texttt{mp\_int}} (Hamming distance):
\index{mp\_hamdist}
\begin{alltt}
mp_err mp_hamdist(const mp_int *a, const mp_int *b, int *hd)
\end{alltt}
The Hamming distance expects both inputs to be of the same size. Thie function allows
for different sizes and assumes leading zeros but returns {\texttt{MP\_OVF}} (overflow error)
to indicate the fact that the inputs are of different bit-sizes. The Levenshtein distance is
not implemented.
...
For divisions known to be exact there is the aptly named function {\texttt{mp\_div\_exact}}.
\index{mp\_div\_exact}
\begin{alltt}
mp_err mp_div_exact (const mp_int *a, const mp_int *b, mp_int *c);
\end{alltt}

In addition to the behaviour of {\texttt{mp\_div}} it checks if the divison is actually
exact and returns \texttt{MP\_VAL} if it is not. This function is significantly
faster\footnote{between 20\% and 40\% depending on size of $n$ and the ratio $kn/n$}
than {\texttt{mp\_div}} if the division is exact.
...
It is not always necessary to use the heavy machinery written for the large cryptographic primes
to handle small primes. It would work, of course, but it would be slow.

Here are some functions to help with those small tasks in no particular order. Here ``small'' means
primes $p < 2^32$. The primecounting function needs about half a second for that limit\footnote{All
benchmarks in this sections done with an Intel i7-2600 (3.40GHz) with 24 Gibibytes RAM}
whereas Kim's primecount\footnote{\url{https://github.com/kimwalisch/primesieve}} does it in about 0.006 seconds.
With {\texttt{MP\_64BIT}} we can try $2^40$ which needs about 2.2 seconds in contrast to 0.08 seconds
with Kim's primecount\footnote{Which \emph{is} the fastest on the market}.

\index{mp\_small\_prime\_primecount}
\begin{alltt}
mp_err mp_small_prime_primecount(mp_digit n, mp_digit *d)
\end{alltt}
Counts the number of primes up to and including the limit {\texttt{n}} and puts the result in {\texttt{d}}.
Does not use the prime-sieve to keep it independent.

\index{mp\_small\_prime\_nthprime}
\begin{alltt}
mp_err mp_small_prime_nthprime(mp_digit n, mp_digit *d)
\end{alltt}
Sets {\texttt{d}} to value of the $n$\textsuperscript{th} prime. Returns {\texttt{MP\_VAL}} if
{\texttt{n}} is large than the limit {\texttt{PRIMECOUNT\_MAX}} which is defined in {\texttt{tommath\_private.h}}.
It is chosen such that the resulting prime is not larger than the data type {\texttt{mp\_digit}} allow.
It is set to $425\,656\,284\,035\,217\,743$, $203\,280\,221$, and $6\,542$ for {\texttt{MP\_64BIT}}, {\texttt{MP\_32BIT}},
and {\texttt{MP\_16BIT}} respectively. It is not the fastest method: with $n = 203\,280\,221$ it needs about 1.2 seconds,
$n = 2 \cdot 203\,280\,221$ about 2.2 seconds and for $n = 4 \cdot 203\,280\,221$ about 3.8 seconds. Kim's primesieve is
about twice as fast for $n = 203\,280\,221$ (single threaded) and about the same for $n = 4 \cdot 203\,280\,221$ and
slower for $n = 8 \cdot 203\,280\,221$ but it supports multithreading which our function does not support.

It uses binary search ($O(\log n)$) with the significant overhead of the primecount function for every check.
The primecounting function uses Lucy Hedgehog's algorithm with a complexity of $\tilde{O}(n^{3/4})$ but it jumps
wildly through large arrays which causes quite the number of (soft) pagefaults.

\subsection{Small Prime Sieve}
A small segmented sieve (unsigned 32-bit max.) is available via three functions.
The type {\texttt{ERAT\_UINT}}, defined (\emph{not} {\texttt{typedef}}) in {\texttt{tommath.h}}
is normally {\texttt{uint32\_t}} but should be checked before use to be sure.

\index{mp\_small\_prime\_sieve\_init}
\begin{alltt}
mp_err mp_small_prime_sieve_init(mp_erat_sieve *sieve, bool warmup);
\end{alltt}
Initialize sieve to default values, build base-sieve, too, if the parameter {\texttt{warmup}} is true
otherwise the first call to either {\texttt{mp\_small\_prime\_sieve\_is\_small\_prime}},
{\texttt{mp\_small\_prime\_sieve\_next\_prime}}, or {\texttt{mp\_small\_prime\_sieve\_prec\_prime}}
do it at first call.

\index{mp\_small\_prime\_sieve\_clear}
\begin{alltt}
mp_err mp_small_prime_sieve_clear(mp_erat_sieve *sieve);
\end{alltt}
Reset sieve and free memory .

\index{mp\_small\_prime\_sieve\_is\_small\_prime}
\begin{alltt}
...
\index{mp\_small\_prime\_sieve\_next\_prime}
\begin{alltt}
mp_err mp_small_prime_sieve_next_prime(ERAT_UINT n, ERAT_UINT *result, mp_erat_sieve *sieve);
\end{alltt}
Sets {\texttt{result}} to the prime {\texttt{p}} larger than or equal to {\texttt{n}} if $p > 2^32 - 5$.
Returns {\texttt{MP\_OVF}} if $p > 2^32 - 5$ as an information about that fact.

\index{mp\_small\_prime\_sieve\_prec\_prime}
\begin{alltt}
mp_err mp_small_prime_sieve_prec_prime(ERAT_UINT n, ERAT_UINT *result, mp_erat_sieve *sieve);
\end{alltt}
Sets {\texttt{result}} to the prime {\texttt{p}} smaller than or equal to {\texttt{n}} if $p > 1$.
...
\section{Valuation}
\index{mp\_valuation\_d}
\begin{alltt}
mp_err mp_valuation_d(const mp_int *a, const mp_digit p, mp_digit *r)
\end{alltt}
It calculates ``the degree of divisibility of a number by a prime number'' to shamelessly
quote Wikipedia\footnote{\url{https://en.wikipedia.org/wiki/Valuation_(algebra)}}.
Basically: it counts the number of times {\texttt{p}} divides {\texttt{a}} and returns
that count in {\texttt{r}}.

This function does not test {\texttt{p}} for primality.

\section{Factorial}
\subsection{Dividers of a factorial}
\index{mp\_factorial\_divisors}
\begin{alltt}
mp_err mp_factorial_divisors(const mp_digit n, const mp_digit p, mp_digit *d) 
\end{alltt}

Computes the Legendre function\footnote{Also known as de Polignac's formula}
$E_{p}(n!)=\sum _{k=1}^{\infty }\left\lfloor \frac{n}{p^{k}}\right\rfloor$
 to find the number of times the prime $p$ divides the factorial $n!$.
A kind of {\texttt{mp\_valuation\_d(n!, p)}} without having to calulate $n!$.

This function does not check if $p$ is prime.
...
\section{Min/Max}
Multivariable functions to find minimum/maximum. Usage is similar to the
usage of e.g., {\texttt{mp\_init\_multi}}. The maximum/minimum gets
copied into the first argument {\texttt{out}}, the last argument, the
sentinel, must be {\texttt{NULL}}.

\index{mp\_min\_multi} \index{mp\_max\_multi}
\begin{alltt}
