...
to handle small primes. It would work, yes, but it would be slow.

Here are some functions to help with those small tasks in no particular order. Here ``small'' means
about $p < 2^32$. The primecounting function needs about half a second for that limit\footnote{All
benchmarks in this sections done with an Intel i7-2600 (3.40GHz) with 24 Gibibytes RAM}
whereas Kim's primecount\footnote{\url{https://github.com/kimwalisch/primesieve}} does it in about 0.006 seconds. With {\texttt{MP_64BIT}} we can try $2^40$ which needs
about 2.2 seconds in contrast to 0.08 seconds with Kim's primecount\footnote{Which \emph{is} the fastest on the market}.
...
{\texttt{n}} is large than the limit {\texttt{PRIMECOUNT\_MAX}} which is defined in {\texttt{tommath_private.h}}.
