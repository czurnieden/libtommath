...
\begin{center}
  ...
  \begin{algorithmic}[1]
    ...
    \Function{Barret\_todecimal}{$a$}
      ...
      \Let{$\rho$}{$1$}\Comment{Index into the buffers $n^l, s^l, m^l$}
      \While{$p \le h$}
        \Let{$n$}{$n^2$}
        \If{$n > a$}
           \Break
      ...
      \State \Call{Barrett\_todecimalRecursion}{$a, n^l, s^l, m^l, \rho - 1, 1, b$}
    \EndFunction
  \end{algorithmic}
\end{center}
...
\multicolumn{1}{c}{\textbf{$\mathbf{n}$}}&
\multicolumn{1}{c}{\textbf{Shift}}&
\multicolumn{1}{c}{\textbf{$\mathbf{M}$}}&
...
\begin{equation}
n = 1 + \floor{\log_2 \left( \floor{\frac{b_a}{2\floor{\log_2 10}}} + 2 \right)  }
\end{equation}
...
\begin{center}
  ...
  \begin{algorithmic}[1]
    \Require{$a > 0$, big-integer, $n^l, s^l, m^l, \rho, L, b$, from Barret\_todecimal}
    \Ensure{$b$ filled with the value of $a$ converted to a decimal number}
    \Function{Barret\_todecimalRecusive}{$a,n^l, s^l, m^l, \rho, L, b$}
    \If{$\rho < 0$}
      \Let{$b$}{$b | a$}\Comment{Convert $a$ to a decimal string and add to $b$. See text for details}
      \RETURN
    \Else
      \Let{$q$}{$\floor{\left(a\cdot m^l_\rho\right) / 2^{s^l_\rho}}$}\Comment{Quotient}
      ...
      \If{$\left(L = 1\right) \AND \left(q = 0\right) $}
        \State \Call{Barret\_todecimalRecusive}{$r,n^l, s^l, m^l, \rho, 1, b$}
      \Else
        \State \Call{Barret\_todecimalRecusive}{$q,n^l, s^l, m^l, \rho, L, b$}
        \State \Call{Barret\_todecimalRecusive}{$r,n^l, s^l, m^l, \rho, 0, b$}
      \EndIf
    \EndIf
    \EndFunction
  \end{algorithmic}
...
\end{equation}
By pre-computing the value at $k=0$ and some clever scaling to replace the expensive division $n^{\left(2^k\right)}$ by a much cheaper shift (with the additional cost of one multiplication and two squarings) we get the code as listed in algorithm \ref{alg:barrettodecimal}.
