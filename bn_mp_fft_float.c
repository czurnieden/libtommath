...
/*
   Even with just 5 bit long chunks it is only good to less
   than ca. 230 60-bit limbs (resulting in a 4096 chunks large float array
   and a bit into the 8192 section that it switches to at 170 limbs )
...
#define MP_DIGIT_BIT_TWELFTH (DIGIT_BIT/12)
#define MP_DIGIT_TWELFTH (1L<< MP_DIGIT_BIT_TWELFTH )
#define MP_DIGIT_MASK (MP_DIGIT_TWELFTH-1)
...
/* Transform multiplicands into floating point numbers with TWELFTH sized digits*/
                 ...
                 mp_int *b, float **fb, int *length)
{
   ...
   /* Digits get split in TWELFTHs, so five times the length is needed*/
   length_needed = ((length_a + length_b ))*12 ;
   ...
   *length = length_needed;
   ...
   fft_array_a = XMALLOC(sizeof(float) * (length_needed + 12));
   if (fft_array_a == NULL) {
      return MP_MEM;
   }


   fft_array_b = XMALLOC(sizeof(float) * (length_needed + 12));
   if (fft_array_b == NULL) {
      return MP_MEM;
   }

   for (i = 0; i<length_needed/12; i++) {
      if (i < length_a) {
         fft_array_a[(12*i)]   = (float) (a->dp[i]                            & MP_DIGIT_MASK);
         fft_array_a[(12*i)+1] = (float)((a->dp[i] >>    MP_DIGIT_BIT_TWELFTH)  & MP_DIGIT_MASK);
         fft_array_a[(12*i)+2] = (float)((a->dp[i] >> (2*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+3] = (float)((a->dp[i] >> (3*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+4] = (float)((a->dp[i] >> (4*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+5] = (float)((a->dp[i] >> (5*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+6] = (float)((a->dp[i] >> (6*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+7] = (float)((a->dp[i] >> (7*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+8] = (float)((a->dp[i] >> (8*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+9] = (float)((a->dp[i] >> (9*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+10] = (float)((a->dp[i] >> (10*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(12*i)+11] = (float)((a->dp[i] >> (11*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
      }
      /* padding a */
      if (i >= length_a) {
         fft_array_a[(12*i)]   = 0.0;
         fft_array_a[(12*i)+1] = 0.0;
         fft_array_a[(12*i)+2] = 0.0;
         fft_array_a[(12*i)+3] = 0.0;
         fft_array_a[(12*i)+4] = 0.0;
         fft_array_a[(12*i)+5] = 0.0;
         fft_array_a[(12*i)+6] = 0.0;
         fft_array_a[(12*i)+7] = 0.0;
         fft_array_a[(12*i)+8] = 0.0;
         fft_array_a[(12*i)+9] = 0.0;
         fft_array_a[(12*i)+10] = 0.0;
         fft_array_a[(12*i)+11] = 0.0;

      }
      if (i < length_b) {
         fft_array_b[(12*i)]   = (float) (b->dp[i]                            & MP_DIGIT_MASK);
         fft_array_b[(12*i)+1] = (float)((b->dp[i] >>    MP_DIGIT_BIT_TWELFTH)  & MP_DIGIT_MASK);
         fft_array_b[(12*i)+2] = (float)((b->dp[i] >> (2*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+3] = (float)((b->dp[i] >> (3*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+4] = (float)((b->dp[i] >> (4*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+5] = (float)((b->dp[i] >> (5*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+6] = (float)((b->dp[i] >> (6*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+7] = (float)((b->dp[i] >> (7*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+8] = (float)((b->dp[i] >> (8*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+9] = (float)((b->dp[i] >> (9*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+10] = (float)((b->dp[i] >> (10*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_b[(12*i)+11] = (float)((b->dp[i] >> (11*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
      }
      /* padding b */
      if (i >= length_b) {
         fft_array_b[(12*i)]   = 0.0;
         fft_array_b[(12*i)+1] = 0.0;
         fft_array_b[(12*i)+2] = 0.0;
         fft_array_b[(12*i)+3] = 0.0;
         fft_array_b[(12*i)+4] = 0.0;
         fft_array_b[(12*i)+5] = 0.0;
         fft_array_b[(12*i)+6] = 0.0;
         fft_array_b[(12*i)+7] = 0.0;
         fft_array_b[(12*i)+8] = 0.0;
         fft_array_b[(12*i)+9] = 0.0;
         fft_array_b[(12*i)+10] = 0.0;
         fft_array_b[(12*i)+11] = 0.0;
      }
   }
   // there is a small problem with divisibility of 2^n and 10, so ...

   rest = (length_needed/12)*12;
   for(i=rest;i<length_needed + 12;i++){
      fft_array_a[i] = 0.0;
      fft_array_b[i] = 0.0;
   }
   ...
   *length = length_needed;
   ...
   for (i = 0; i<length_needed/5; i++) {
      if (i < length_a) {
         ...
         fft_array_a[(5*i)+1] = (float)((a->dp[i] >>    MP_DIGIT_BIT_TWELFTH)  & MP_DIGIT_MASK);
         fft_array_a[(5*i)+2] = (float)((a->dp[i] >> (2*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(5*i)+3] = (float)((a->dp[i] >> (3*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
         fft_array_a[(5*i)+4] = (float)((a->dp[i] >> (4*MP_DIGIT_BIT_TWELFTH)) & MP_DIGIT_MASK);
      }
      if (i >= length_a) {
         fft_array_a[(5*i)]   = 0.0;
...
int mp_fft_to_dp_float(float *fft_array, mp_int *a,int length)
{
   ...
   for (i=0; i<length; i++) {
      ...
      if (temp >= MP_DIGIT_TWELFTH) {
         carry = temp / (mp_word)MP_DIGIT_TWELFTH;
         temp  = temp % (mp_word)MP_DIGIT_TWELFTH;
      }
...
#include <fenv.h>
   ...
   /* re-marry the digits */
   for (i=0,j=0; j<new_length; i++,j+=12) {
      a->dp[i]  = (mp_digit)(roundf(fft_array[j+11]))  & MP_DIGIT_MASK;
      a->dp[i] <<= MP_DIGIT_BIT_TWELFTH;
      a->dp[i] |= (mp_digit)(roundf(fft_array[j+10]))  & MP_DIGIT_MASK;
      a->dp[i] <<= MP_DIGIT_BIT_TWELFTH;
